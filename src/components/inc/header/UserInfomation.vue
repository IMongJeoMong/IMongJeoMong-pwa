    ...
    <div id="topInfoBox">
        <div class="topInfo">
            <div class="level_gauge_bar">
                <div class="player_level" v-if="getSelectMong">{{Math.floor(getSelectMong.exp / 1000)}}</div>
  ...
  async mounted() {
    setTimeout(() => {
      if (this.$refs.levelGaugeBarProgress) {
        ...
        this.$refs.levelGaugeBarProgress.style.width =
          this.getSelectMong.expPercent + "%";
      }
    }, 500);
  },
};
